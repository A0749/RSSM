...
"sliderImages": []
  ...
  async function fetchCaptions() {
    try {
        ...
        return {}; // Avoid breaking the script
    }
}
...
async function init() {
                    ...
                    <div class="swiper-wrapper">
                        ${subject.sliderImages.map(img => `
                            <div class="swiper-slide">
                                <img src="${img}" alt="Slide image" class="swiper-image" data-src="${img}">
                            </div>
                        `).join("")}
                    </div>
    ...
    // Initialize Swiper for each subject
    subjects.forEach(subject => {
        new Swiper(`.swiper-${subject.id}`, {
            loop: true,
            slidesPerView: 1,
            spaceBetween: 10,
            breakpoints: {
            ...
            pagination: {
                el: `.swiper-${subject.id} .swiper-pagination`,
                clickable: true,
            },
            navigation: {
    ...
    // Add event listeners to Swiper images AFTER they are created
    setTimeout(() => {
        document.querySelectorAll(".swiper-image").forEach((image) => {
            image.addEventListener("click", (e) => {
                const clickedImgSrc = e.target.dataset.src; // Get image source
                slickModeInstance.open([...document.querySelectorAll(".swiper-image")], clickedImgSrc, captions);
            });
        });
    }, 500); // Ensure DOM is updated before attaching events
}
